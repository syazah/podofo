...
async function handleSubmit(data: BatchSubmitJobData) {
  ...
  const documents = await docDB.getDocumentsByLotId(lotId);
  ...
  if (state === JobState.JOB_STATE_SUCCEEDED) {
    if (stage === "classification") {
      ...
      if (counts.pending === 0 && counts.classified + counts.failed === counts.total) {
        ...
        const allDocs = await docDB.getDocumentsByLotId(lotId);
    ...
    for (const docId of documentIds) {
      try {
        await docDB.updateDocumentStatus(docId, "failed", `Batch job failed: ${state}`);
      } catch (err) {
