...
export class LotDB {
  private static instance: LotDB
  private constructor() { }
  public static getInstance(): LotDB {
    if (!LotDB.instance) {
      LotDB.instance = new LotDB()
    }
    return LotDB.instance;
  }

  createLot = async (totalFiles: number) => {
  ...
  updateLotStatusOnly = async (lotId: string, status: string) => {
    const { data, error } = await supabaseAdmin
      .from("lots")
      .update({
        status,
